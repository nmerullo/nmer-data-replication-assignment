...
plot(as.factor(dengue$Severe_Dengue), dengue$PCV_day_1, xlab = "Severe Dengue", ylab = "PCV Day 1", col = "hotpink")
plot(as.factor(dengue$Severe_Dengue), dengue$PCV_day_2, xlab = "Severe Dengue", ylab = "PCV Day 2", col = "yellow")
```
...
# p.values <- pvalString(p.values, format = "default")
...
```{r columns}
newtable1 <- as.data.frame(cbind(Severe_Means, Severe_SDs, Non_Severe_Means, Non_Severe_SDs, p.values))
newtable1
```

I wanted to try to make my table look a little more professional, so I found this package {pixiedust} which can easily clean up the cell values in tables, along with a lot of cool other functions.

```{r naming-rows}
library(pixiedust)
newtable1 <- dust(newtable1) %>% 
  sprinkle(col = 1:4, round = 2) %>% #round values in columns 1:4 to 2 decimal places
  sprinkle(col = 5, fn = quote(pvalString(value))) %>% #format pvalue without scientific notation!
